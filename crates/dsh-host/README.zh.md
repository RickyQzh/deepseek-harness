...
DeepSeek Harness Rust 进程的 GUI 宿主 crate。它只在 `127.0.0.1` 上绑定 axum 监听器（`HostBind`；其它 `listen_host` 均为 `HostError`），提供 SPA dist 与 `/plugins` bundle，通过 `RpcHandler` 分发一元 `POST /api/<method>` 的 JSON `client-request` 信封（点分 `RpcMethodMap` 与第 7 阶段 slash Remote），并经 WebSocket 承载 mux/host GUI 下行。端口 `0` 由操作系统分配。`ListeningHost::local_addr` 报告实际绑定地址；`shutdown` 停止 accept 并等待进行中的请求结束。`ListeningHost::hub` / `HostState::hub` 是用于发布下行帧的可克隆 `DownlinkHub`。

每个 `/api` 请求都会用 Host、Origin、`sec-fetch-site` 与 `trustedHosts` 运行 `is_trusted_api_request`。不受信任的 `/api` 返回 HTTP 403，正文为 `forbidden`。没有 CORS，也没有 TLS。

`POST /api/<method>` 要求 `Content-Type: application/json`（忽略 `;` 之后的参数）；其它媒体类型返回 HTTP 415，正文为 `content type must be application/json`。不是 `client-request` 的 JSON，或其 `method` 与 `/api/` 之后的路径后缀不相等，返回 HTTP 400。解析之后，特权 dotted 方法（见 `is_privileged_method`）会按 `trustedHosts` 为空再检查一次：`privileged_requires_loopback` 必须为 true，因此即使 Host 列在 `trustedHosts` 中，非 loopback Host 仍是 403。slash 路径不在该特权集合中。若 `/api/` 之后的路径含 `/`，则走 slash 拦截器：只安装 payload 为 `{ "args": { … } }` 的 `commands/list` 与 `commands/execute`（list 可以为 `{ "commands": [] }`；execute 未命中为 `unknown-command`）；其它 slash 路径为 HTTP 404。专用的 `POST /api/respond` 以及 `GET`/`HEAD` `/api/events.mux` 与 `/api/events.host` 不进入拦截器。未知 dotted 方法是 HTTP 404（载体），不是 `RpcResult::err`。已挂载处理函数的业务结果（含错误）是 HTTP 200 + `server-response`。本 crate 的 `StubHandler` 只回答 `host.describe`（`version` 为 `0.0.1`，`cwd` 来自 `DSH_CWD` 或 `current_dir`，`attachedSessions` 为 `0`，`canOpenPath` 为 `false`）。`SessionHandler` 回答 `host.describe`（`attachedSessions` 取自活 registry 长度，可选的 `provider`/`model` 取自第一个 LLM（大语言模型） provider 或 lookup 默认值），以及 dotted `session.list` / `create` / `history` / `prompt` / `cancel` / `models` / `selectModel` / `rename` / `updateQueue` / `fork` / `search` / `attachment`。`GuiHandler` 包装 `SessionHandler`，并额外回答 `host.listDirectory` / `createDirectory` / `pickDirectory` / `openPath`、`workspace.*`、`skill.list`、`agentPreset.*`、dotted `goal.*`、`llm.providers` / `models` / `discoverModels`、`settings.*`、`credentials.*`、`subagent.list` 以及 slash Remote。`AgentLookup::agent_for` 复用活句柄，或对 JSONL 冷恢复做单飞；头的 `origin` 为 `subagent` 或已设置 `parent_session` 时返回 `agent-busy`（带父会话的非 subagent 子会话会误伤，直到 origin 被如实记录）。`POST /api/respond` 使用相同的信任围栏与 JSON Content-Type 检查，然后返回没有 `type` 字段的 JSON `RpcReceipt`：`rpcId` 在 `RespondTable` 中的 `client-response` 为 `{ "accepted": true }`；未知 id 为 `not-pending`；其它 JSON 为 `bad-response`。
