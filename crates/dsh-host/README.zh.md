...
DeepSeek Harness Rust 进程的 GUI 宿主 crate。它只在 `127.0.0.1` 上绑定 axum 监听器（`HostBind`；其它 `listen_host` 均为 `HostError`），提供 SPA dist 与 `/plugins` bundle，并通过 `RpcHandler` 分发一元 `POST /api/<dotted>` 的 JSON `client-request` 信封。端口 `0` 由操作系统分配。`ListeningHost::local_addr` 报告实际绑定地址；`shutdown` 停止 accept 并等待进行中的请求结束。
...
对 `/api/events.mux` 与 `/api/events.host` 的 `GET`/`HEAD` 返回 HTTP 426，并带 `Upgrade: websocket`。尚未实现 WebSocket 升级。`GET`/`HEAD` `/plugins/<id>/client.js` 使用静态辅助函数；含 `/` 的 scoped 图 id 会映射到扫描得到的目录名，不会当作单个路径段传入。其它 `GET`/`HEAD` 路径使用 `serve_spa`。这些静态路径上的其它 HTTP 方法返回 HTTP 405。
...
- `/api/events.mux` 与 `/api/events.host` 上的 WebSocket 升级尚未实现；网络 GET/HEAD 仍返回 426。
