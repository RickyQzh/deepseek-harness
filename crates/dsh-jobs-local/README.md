...
`LocalJobRegistry::new(max_concurrent_per_owner)` sets the active-job cap. Config `maxConcurrentJobsPerOwner` is a positive integer and defaults to `10`. `start()` holds the registry mutex through the active-count check, `run()`, and insert, so concurrent starts cannot exceed the cap. `run` must not re-enter the registry. All unowned jobs share one bucket. Ids are `<kind>-N` with per-kind process-global counters: the first bash job is `bash-1` even if a subagent job already exists. A panic from `run` is not caught; nothing is registered until `run` returns. Dropping a `wait` future uncounts the waiter so a cancelled wait cannot suppress `on_job_done`.
