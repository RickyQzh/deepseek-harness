...
`LocalFileSystem` joins relative paths against `cwd` (or a per-call cwd), realpaths the deepest existing ancestor plus remaining suffix, and uses that canonical string for both `target_key` and `display_path`. `read_text` accepts regular UTF-8 files, rejects NUL in the first 8192 bytes and invalid UTF-8, and normalizes `\r\n` to `\n`. `stream_text` uses the same regular-file, first-8192 NUL, and UTF-8 rules and yields raw decoded chunks without rewriting CRLF. `file_url` is the POSIX `file:` URI of `process_path`, with percent-encoding matching Node `pathToFileURL` on unix. `write_text` publishes through a `0o600` sibling temp file then `rename`; omit the intent for unconditional create-or-overwrite. `edit_text` applies a literal replace on LF-normalized text and restores CRLF when that was the original style. Version tokens are `{dev}:{ino}:{size}:{mtime_nsec}:{ctime_nsec}`.
