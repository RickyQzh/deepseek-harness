...
Harness 身份是顺序为 −100 的 `harness:identity` 段落。部署 persona 是顺序为 0 的 `deployment:persona`。`TOOL_ORDER_REST`（`<unlisted-tools>`）标记未列出的工具按字典序插入的位置。快照同一性（渲染文本未变化时跳过新的用户消息）由 `dsh-agent-loop` 应用，而不是本 crate。`Clone` 共享实时注册表，因此之后的 `section` 调用对 `assemble` 仍然可见。
