...
本 crate 不是 YAML 插件，也不导出 `register`。后端在后续 crate 中实现 `CompactionEngine`；YAML 不得列出 `@deepseek-ai/dsh-compaction`。
...
- 锁、表层替换、摘要，以及 `compaction/start|summary|end` 载荷不在本 crate 实现。
