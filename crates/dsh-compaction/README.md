...
This crate is not a YAML plugin and does not export `register`. Backends in a later crate implement `CompactionEngine`; YAML must not list `@deepseek-ai/dsh-compaction`.
...
- Lock, surface replace, summarization, and `compaction/start|summary|end` payloads are not implemented here.
