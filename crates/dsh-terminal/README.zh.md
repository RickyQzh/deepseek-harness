...
`TerminalSessionId` 是包住 `dsh_brand::Branded<TerminalSessionIdTag>` 的本地 newtype。`new` 为 crate 私有；调用方通过 `spawn` 铸造 id，依次发出 `pty-1`、`pty-2`、…。工具通过 `from_arg` 从 JSON 重建已有 id（空字符串失败；不做格式检查）。Owner 是 `dsh_session::SessionId`。`has_owner_activity` 从尚未发布的 spawn 预留一直为 true，直到 close，没有发布空隙。`spawn`、`kill` 和 `signal` 是异步的：它们放下内部 mutex，等待后端的对象安全 boxed future（`Pin<Box<dyn Future + Send>>`），然后再加锁。第一次 `kill` 占用 close 槽位，并在 `tokio` 任务中运行后端 close、取消发布和等待者完成，因此第一个 `kill` future 被丢弃时，后续 `kill` 等待者仍会在该 close 结束后结算。并发 `kill` 返回 `false`；取消发布之后的迟到 `kill` 为 `unknown PTY session {id}`。snapshot 的 spawn、close 和 signal 以 `Box::pin(std::future::ready(...))` 完成。`TerminalSessionService` 实现 `Clone`，因此调用方在 `.await` 之前从提供的 `Mutex<TerminalSessionService>` 中克隆出来。`start_send` 互斥（`SEND_ACTIVE`，`PTY session {id} already has an active send`）。缺失 id 为 `NO_SESSION`（`unknown PTY session {id}`）。他人 owner 为 `FOREIGN_SESSION`，消息包含该 id。空的后端类型会失败；重复类型为 `DUPLICATE_BACKEND`（`a PTY backend named "{type}" is already registered`）；spawn 时缺少类型为 `NO_BACKEND`（`no PTY backend registered for "{type}"`）。
