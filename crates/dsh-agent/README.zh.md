...
YAML 名称为 `@deepseek-ai/dsh-agent`，注入 `llm`、`tools` 和 `systemPrompt`，并提供 `agents`。该插件保存内核提供的同一把 `Arc<Mutex<LlmRuntime>>` 与 `Arc<Mutex<ToolRuntime>>`；兄弟插件在这些互斥锁上的 `register_adapter` 或 `register` 对 `AgentRegistry::list_providers` 仍然可见。`register_spine_plugins` 将该插件与 credentials、llm（含 mock、replay 与 DeepSeek）、tools、system-prompt 以及 JSONL 会话存储一并注册。
