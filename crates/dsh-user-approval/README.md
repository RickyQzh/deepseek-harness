...
Fail-closed one-shot approval for the Rust host. `ApprovalService::request` requires an open turn; it appends `approval/asked`, runs the `approval/request` waterfall with default `Unavailable`, then appends `approval/decided`. Policy `Never` still writes that pair but returns `Rejected` without calling answerers. An idle `request` returns `ApprovalError::Idle` and appends nothing.
...
- The waterfall payload is `ApprovalOutcome` only; answerers do not receive the `ApprovalRequest` as a typed value.
