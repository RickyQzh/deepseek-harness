...
`register` mounts YAML `@deepseek-ai/dsh-acp` (`dsh_boot::PLUGIN_ACP`). `register_acp_plugins` calls `register`. Plugin setup injects `agents` and `sessions`, binds stdin/stdout, installs the permission listener, provides `acpServer`, and does not serve; `ACP_YAML` is the bundled composition (mock text `acp-ok`, approval `ask`, no auto-approve). Config requires string `provider` and `model`. This crate does not depend on `dsh-sdk-protocol`, `dsh-sdk-jsonrpc-server`, `dsh-base`, `dsh-host`, `dsh-cli`, `dsh-headless`, or `dsh-subagent`. `dsh-agent` must not depend on this crate.
...
`session/prompt` waits for whole-agent idle, emits committed `agent_message_chunk` text, and reports `end_turn` (token-limit included). `session/cancel` is a no-op for unknown ids and settles an in-flight prompt as `cancelled`; a second prompt while one is in flight is `-32602`. One connection owns several sessions with independent prompt slots. Stdio EOF and `quiesce` cancel in-flight prompts and unregister those sessions; continuable descendants are not drained. After EOF, `serve` waits for spawned inbound request handlers to write their JSON-RPC responses before returning, so a one-line `initialize` then stdin close still emits the handshake.
