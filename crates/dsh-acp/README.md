...
`session/prompt` waits for whole-agent idle, emits committed `agent_message_chunk` text, and reports `end_turn` (token-limit included). `session/cancel` is a no-op for unknown ids and settles an in-flight prompt as `cancelled`; a second prompt while one is in flight is `-32602`. One connection owns several sessions with independent prompt slots. Stdio EOF and `quiesce` cancel in-flight prompts and unregister those sessions; continuable descendants are not drained.
