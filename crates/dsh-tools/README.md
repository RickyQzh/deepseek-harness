...
`plugin::register` provides `tools` as an empty native `ToolRuntime`. `register` overwrites a duplicate name; `try_register` leaves the existing tool and returns `RegisterError::Duplicate`; `unregister` removes a name and returns whether it was present. `registered_names` returns the sorted model-facing names currently registered.
