...
`--profile` 为必填。已实现的 profile 为 `headless` 与 `web`。`dsh web` 是 `--profile web` 的别名。argv[1] 为 `plugin`，以及任何未实现的 `--profile`，都会向 stderr 打印 `dsh: {verb} is not implemented` 并以退出码 2 退出。本二进制不会 spawn Node。
