...
`--profile` is required. Implemented profiles are `headless`, `web`, and `acp`. `dsh web` is an alias for `--profile web`. `dsh acp` is an alias for `--profile acp`. `plugin` as argv[1], and any unimplemented `--profile`, print `dsh: {verb} is not implemented` to stderr and exit 2. This binary does not spawn Node.
...
ACP: invoke `dsh acp` or `dsh --profile acp` with optional repeatable `--patch`. A leftover positional task is a usage error (`acp takes no task argument`). `--port`, `--dist`, `--host`, and `--trusted-host` are ignored; ACP binds no TCP port. Config YAML is `$DSH_CORDIS_CONFIG` when that variable is set and non-empty, otherwise the bundled `ACP_YAML` (`acp.cordis.yml`) from `dsh-acp`. The launcher registers spine, execution, base, and `dsh_acp::register_acp_plugins`, not headless or host plugins, injects `acpServer`, and serves NDJSON JSON-RPC on stdio until EOF. Diagnostics go to stderr; stdout is ACP frames only.
