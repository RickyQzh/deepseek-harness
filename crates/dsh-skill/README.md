...
`register_skill` provides `skills` as a `SkillRegistry`. `inject::<SkillRegistry>()` yields `Arc<SkillRegistry>`, so `register_provider`, `list`, and `get` take `&self` with an interior mutex. Duplicate provider names fail. Same-name skills resolve by lower rank, then provider registration order; `list` returns the winning summaries sorted by name. `get` overlays that winning summary (including discovery `source`) onto the loaded body. This phase is one global layer (no isolate realms). Skill names match `/^[a-z0-9]+(?:-[a-z0-9]+)*$/`.
