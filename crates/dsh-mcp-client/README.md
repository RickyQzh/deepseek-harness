...
`register` mounts YAML `@deepseek-ai/dsh-mcp-client` (`dsh_boot::PLUGIN_MCP_CLIENT`). `register_mcp_plugins` calls `register`. Plugin setup parses stdio config (unknown keys fail load; `transport: streamable-http` fails with `mcp-client: streamable-http is not supported in Phase 8 item 2`), injects `tools`, reserves `serverName` on that runtime, starts the reconnect supervisor, and awaits the first generation `ready` before activation. A duplicate live `serverName` fails load. `failOnStartupError` defaults to false: a failed first attempt logs and leaves the supervisor looping; `true` returns `SetupFailed`. Config, reservation, HTTP, and reconnect-policy errors always fail load. `resolve_reconnect_policy` uses prefix `mcp-client({serverName}): reconnect` and defaults `enabled` true, `initialDelayMs` 500, `maxDelayMs` 30000, `maxAttempts` 10. After a successful initialize, child exit or read EOF reconnects with backoff `min(maxDelayMs, initialDelayMs * 2^(failedAttempts-1))` when enabled; uptime since `connectedAt` of at least `maxDelayMs` resets `failedAttempts` to 0 before incrementing. During an outage the last generation stays registered and calls fail. After `maxAttempts` consecutive failures the supervisor unregisters owned public names and stops. `reconnect.enabled: false` does not respawn. Dispose cancels backoff, kills the child, waits up to 5000 ms, unregisters remaining names, and releases `serverName`.
