...
`JsonlSessionStore` 将未压缩、未打包的 JSONL 写入 `{root}/{sessionId}/session.jsonl`（`SessionId::as_str()`，没有 `--<project>--` 段），以便与 Python SDK 和 jsonrpc fixture（测试前置数据）一致；`flush` 可以整文件重写，`load` 经 `decode_session_log` 再 `Session::from_events` 读回。`list_ids` 返回 `root` 下含 `session.jsonl` 的目录名；缺失的 `root` 得到空列表。`from_env` 在 `DSH_SESSION_ROOT` 已设置且非空时使用该目录，否则使用 `{DSH_HOME}/sessions`。
