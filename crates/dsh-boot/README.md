...
This crate does not register product plugins. Callers `register` setups on a `PluginRegistry` by YAML `name`. Spine composition is `dsh_agent::register_spine_plugins`.

## Known Limitations and Deferred Work

- Product plugin setups are not in this crate; `dsh_agent::register_spine_plugins` owns the spine YAML names.
