...
本 crate 不注册产品插件。调用方按 YAML `name` 在 `PluginRegistry` 上 `register` setup。spine 组合入口是 `dsh_agent::register_spine_plugins`。

## 已知限制与暂缓事项

- 产品插件 setup 不在本 crate 中；spine 的 YAML 名称由 `dsh_agent::register_spine_plugins` 注册。
