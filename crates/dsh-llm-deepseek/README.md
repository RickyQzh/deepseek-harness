...
`plugin::register` injects `llm` and `credentials`, registers `DeepSeekAdapter` on `deepseek-official`, and resolves the API key per request. Config `apiKeyEnv` defaults to `DEEPSEEK_API_KEY`.
