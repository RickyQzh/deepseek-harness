...
Phase 5 jsonrpc scenarios on the Rust bin are `text-turn` and `bash-tool` only. `subagent-spawn-in-process` and `persistent-tools` stay on the Node driver until later phases. Existing `examples/headless-agent` scenarios other than a new minimal keyless path stay on the Node bin until Phase 6/8.
...
| jsonrpc `subagent-spawn-in-process` | Vitest `sdk.snapshot.ts` | Node only | `examples/jsonrpc-agent/tests/snapshots/subagent-spawn-in-process/` |
| jsonrpc `persistent-tools` | Vitest `sdk.snapshot.ts` | Node only | `examples/jsonrpc-agent/tests/snapshots/persistent-tools/` |
| headless compaction-recovery, provider-retry, pty-tools, ralph-loop, goal-tools, advanced-toolchain, subagent-settlement, headless-profile | existing Vitest | Node only | existing dirs |
| headless minimal keyless | new path in Phase 5 (crate test + built `dsh --profile headless`) | Rust | do not copy those fixture dirs |
...
**Spawn the Rust bin for every existing headless and jsonrpc scenario in Phase 5.** Rejected: those scenarios require Phase 6/8 capabilities (compaction, subagents, PTY, persistent tools). Node remains their driver.
...
- Phase 5 names `text-turn` and `bash-tool` as the jsonrpc Rust subset and names the headless scenarios that stay on Node.
...
A reviewer may treat skipped notification-JSONL equality on the Rust path as weakening the snapshot gate. The Node default still pins the full transcript; the Rust path pins the shipping SDK result, idle status, persist path, and `serverInfo.name` until Phase 6 composition can match the Node event stream.
